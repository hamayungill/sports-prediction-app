...
CREATE VIEW sport.baseball_games_stage_status_view AS WITH baseball_games_data AS (
    SELECT g.game_id,
        ...
        s2.sport_name,
        s2.feature_flags,
        (s2.feature_flags->>'time_zone') AS time_zone
    FROM sport.games g
    ...
    WHERE lower(s2.sport_name) = 'baseball'::text
),
game_stages AS (
    SELECT baseball_games_data.data->>'id'::text AS api_game_id,
        (
            (
                (baseball_games_data.data->>'date'::text)::timestamp WITH time zone
            ) AT TIME ZONE 'UTC'
        ) AS start_date_utc,
        (
            (
        ...
        ) AT TIME ZONE baseball_games_data.time_zone AS start_date_local,
        baseball_games_data.data->>'stage'::text AS stage
    FROM baseball_games_data
),
stage_start_dates AS (
    SELECT baseball_games_data.data->>'stage'::text AS stage,
        (
            min(baseball_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC'
        ) AS stage_start_date_utc,
        (
            min(baseball_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC'
        ) AT TIME ZONE baseball_games_data.time_zone AS stage_start_date_local
    FROM baseball_games_data
    GROUP BY baseball_games_data.data->>'stage'::text,
...
days_weeks_status AS (
    SELECT gd.api_game_id,
        ...
        CASE
            WHEN lower(gd.data->>'status'::text) = 'cancelled'::text
            AND dwi.start_date_local >= (CURRENT_TIMESTAMP AT TIME ZONE 'UTC') THEN 'scheduled'
...
CREATE VIEW sport.basketball_games_stage_status_view AS WITH basketball_games_data AS (
    ...
    FROM sport.games g
        ...
        JOIN sport.leagues l ON s.league_id = l.league_id
        JOIN sport.sports s2 ON l.sport_id = s2.sport_id
    WHERE lower(s2.sport_name) = 'basketball'
),
stage_start_dates AS (
    SELECT basketball_games_data.data->>'stage'::text AS stage,
        (
            min(
        ...
        (
            min(
                (basketball_games_data.data->'date'::text)->>'start'::text
            )::timestamp WITH time zone AT TIME ZONE 'UTC' AT TIME ZONE basketball_games_data.time_zone
        ) AS stage_start_date_local
    FROM basketball_games_data
    GROUP BY basketball_games_data.data->>'stage'::text,
        basketball_games_data.time_zone
),
game_stages AS (
    SELECT basketball_games_data.data->>'id'::text AS api_game_id,
        (
            (basketball_games_data.data->'date'::text)->>'start'::text
        )::timestamp WITH time zone AT TIME ZONE 'UTC' AS start_date_utc,
        (
            (basketball_games_data.data->'date'::text)->>'start'::text
        )::timestamp WITH time zone AT TIME ZONE 'UTC' AT TIME ZONE basketball_games_data.time_zone AS start_date_local,
...
days_weeks_status AS (
    SELECT gd.api_game_id,
        ...
        (gd.data->'teams'::text)->'away'::text AS away,
        gd.sport_id,
        ((gd.feature_flags->'pickem'::text)->>'day'::text)::boolean AS pickem_day,
        (
            (gd.feature_flags->'pickem'::text)->>'week'::text
        )::boolean AS pickem_week,
        gd.league_id,
        gd.sport_name
    FROM basketball_games_data gd
        JOIN days_weeks_info dwi ON gd.game_id = dwi.game_id
)
SELECT days_weeks_status.api_game_id,
    days_weeks_status.game_id,
...
CREATE VIEW sport.mma_games_stage_status_view AS WITH mma_games_data AS (
    ...
    FROM sport.games g
        ...
        JOIN sport.leagues l ON s.league_id = l.league_id
        JOIN sport.sports s2 ON l.sport_id = s2.sport_id
    WHERE lower(s2.sport_name) = 'mma'
),
stage_start_dates AS (
    SELECT mma_games_data.data->>'stage'::text AS stage,
        (
            min(mma_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC'
        ) AS stage_start_date_utc,
        (
            min(mma_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC' AT TIME ZONE mma_games_data.time_zone
        ) AS stage_start_date_local
    FROM mma_games_data
...
game_stages AS (
    SELECT mma_games_data.data->>'id'::text AS api_game_id,
        (
            (mma_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC'
        ) AS start_date_utc,
        (
            (mma_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC' AT TIME ZONE mma_games_data.time_zone
        ) AS start_date_local,
...
days_weeks_status AS (
    SELECT gd.api_game_id,
        ...
        gd.data->>'status'::text AS api_status,
        CASE
            WHEN lower(gd.data->>'status'::text) = 'cancelled'
            AND dwi.start_date_utc >= (CURRENT_TIMESTAMP AT TIME ZONE 'UTC') THEN 'scheduled'
            ELSE lower(gd.data->>'status'::text)
        END AS STATUS,
        dwi.start_date_utc AS start_date,
        dwi.stage_start_date_utc AS stage_start_date,
...
CREATE VIEW sport.soccer_games_stage_status_view AS WITH soccer_games_data AS (
    ...
    FROM sport.games g
        ...
        JOIN sport.leagues l ON s.league_id = l.league_id
        JOIN sport.sports s2 ON l.sport_id = s2.sport_id
    WHERE lower(s2.sport_name) = 'soccer'
),
stage_start_dates AS (
    SELECT soccer_games_data.data->>'stage'::TEXT AS stage,
        (
            min(soccer_games_data.data->>'date'::TEXT)::timestamp WITH time ZONE AT TIME ZONE 'UTC'
        ) AS stage_start_date_utc,
        (
            min(soccer_games_data.data->>'date'::TEXT)::timestamp WITH time ZONE AT TIME ZONE 'UTC' AT TIME ZONE soccer_games_data.time_zone
        ) AS stage_start_date_local
    FROM soccer_games_data
    GROUP BY soccer_games_data.data->>'stage'::TEXT,
        soccer_games_data.time_zone
),
game_stages AS (
    SELECT soccer_games_data.data->>'id'::TEXT AS api_game_id,
        (
            (soccer_games_data.data->>'date'::TEXT)::timestamp WITH time ZONE AT TIME ZONE 'UTC'
        ) AS start_date_utc,
        (
            (soccer_games_data.data->>'date'::TEXT)::timestamp WITH time ZONE AT TIME ZONE 'UTC' AT TIME ZONE soccer_games_data.time_zone
        ) AS start_date_local,
        soccer_games_data.data->>'stage'::TEXT AS stage
    FROM soccer_games_data
),
days_weeks_info AS (
    SELECT gs.api_game_id,
...
SELECT soc.api_game_id,
    ...
    soc.valid_day,
    soc.valid_week
FROM sport.soccer_games_stage_status_view soc WITH DATA;
