CREATE OR REPLACE VIEW sport.nba_games_stage_status_view AS WITH nba_games_data AS (
        SELECT g.game_id,
            g.season_id,
            g.api_game_id,
            g.data,
        ...
        WHERE (
                g.api_source_id IN (
                    ...
                    WHERE lower(as2.api_source_name) ~~* '%nba%'::text
                )
            )
    ),
    stage_start_dates AS (
        SELECT nba_games_data.data->>'stage'::text AS stage,
            (
                min(
                    (nba_games_data.data->'date'::text)->>'start'::text
                )::timestamp WITH time zone AT TIME ZONE 'UTC'::text
            ) AS stage_start_date
        FROM nba_games_data
        GROUP BY (nba_games_data.data->>'stage'::text)
    ),
    game_stages AS (
        SELECT nba_games_data.data->>'id'::text AS api_game_id,
            (
                (
                    (
                        (nba_games_data.data->'date'::text)->>'start'::text
            ...
            nba_games_data.data->>'stage'::text AS stage
        FROM nba_games_data
    ),
    days_weeks_info AS (
    ...
    days_weeks_status AS (
        SELECT g.api_game_id,
            ...
            lower(g.data->>'status'::text) AS STATUS,
            dwi.start_date,
