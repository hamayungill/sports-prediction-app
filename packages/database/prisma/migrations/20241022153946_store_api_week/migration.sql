...
CREATE VIEW sport.football_games_stage_status_view AS WITH football_games_data AS (
    SELECT
        ...
        , s2.sport_name
        , s2.feature_flags
        , (g.start_date::timestamp without time zone AT TIME ZONE 'UTC'::text) AS start_date
    ...
    WHERE
        ...
        AND (((g.data -> 'teams'::text) -> 'home'::text) ->> 'name'::text) IS NOT NULL
        AND lower(((g.data -> 'teams'::text) -> 'home'::text) ->> 'name'::text) <> 'tba'::text
        AND (((g.data -> 'teams'::text) -> 'away'::text) ->> 'name'::text) IS NOT NULL
        AND lower(((g.data -> 'teams'::text) -> 'away'::text) ->> 'name'::text) <> 'tba'::text
)
...
, stage_start_dates AS (
    SELECT
        ...
        , min((football_games_data.start_date AT TIME ZONE football_games_data.time_zone)) AS stage_start_date_local
    FROM football_games_data
    GROUP BY (football_games_data.data ->> 'stage'::text), football_games_data.time_zone
)
...
, days_weeks_info AS (
    SELECT
        ...
        , (gs.start_date_local::date - ssd.stage_start_date_local::date) / 7 + 1 AS week_of_stage
...
, days_weeks_status AS (
    SELECT
        ...
        , ((gd.feature_flags -> 'pickem'::text) ->> 'week'::text)::boolean AS pickem_week
        , gd.league_id
...
SELECT
    ...
    , CASE
        ...
        ELSE min(
            ...
            OVER (
                PARTITION BY days_weeks_status.stage, days_weeks_status.day_of_stage
    ...
    , CASE
        ...
        ELSE min(
            ...
            OVER (
                PARTITION BY days_weeks_status.stage, days_weeks_status.week_of_stage
                ORDER BY days_weeks_status.start_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
            )
        IS NULL
    END AS valid_week
FROM days_weeks_status
ORDER BY days_weeks_status.stage, days_weeks_status.start_date;
...
, stage_start_dates AS (
    SELECT
        ...
        , min((baseball_games_data.start_date AT TIME ZONE baseball_games_data.time_zone)) AS stage_start_date_local
    FROM baseball_games_data
    GROUP BY (baseball_games_data.data ->> 'stage'::text), baseball_games_data.time_zone
)
...
, days_weeks_status AS (
    SELECT
        ...
        , CASE
            WHEN
                lower(gd.data ->> 'status'::text) = 'cancelled'::text AND dwi.start_date_local >= (current_timestamp AT TIME ZONE 'UTC'::text)
                THEN 'scheduled'::text
...
, stage_start_dates AS (
    SELECT
        ...
        , min((basketball_games_data.start_date AT TIME ZONE basketball_games_data.time_zone)) AS stage_start_date_local
    FROM basketball_games_data
    GROUP BY (basketball_games_data.data ->> 'stage'::text), basketball_games_data.time_zone
)
...
, stage_start_dates AS (
    SELECT
        ...
        , min((mma_games_data.start_date AT TIME ZONE mma_games_data.time_zone)) AS stage_start_date_local
    FROM mma_games_data
    GROUP BY (mma_games_data.data ->> 'stage'::text), mma_games_data.time_zone
)
...
, days_weeks_status AS (
    SELECT
        ...
        , CASE
            WHEN
                lower(gd.data ->> 'status'::text) = 'cancelled'::text AND dwi.start_date_utc >= (current_timestamp AT TIME ZONE 'UTC'::text)
                THEN 'scheduled'::text
            ELSE lower(gd.data ->> 'status'::text)
        END AS status
        , (gd.data -> 'teams'::text) -> 'home'::text AS home
...
, game_stages AS (
    SELECT
        ...
        , CASE
            WHEN
                trim(BOTH ' '::text FROM split_part(soccer_games_data.data ->> 'stage'::text, '-'::text, 2)) ~ '^\d+$'::text
                THEN split_part(soccer_games_data.data ->> 'stage'::text, '-'::text, 1)
...
, stage_start_dates AS (
    SELECT
        ...
        , min((soccer_games_data.start_date AT TIME ZONE soccer_games_data.time_zone)) AS stage_start_date_local
    FROM soccer_games_data
    GROUP BY (
        CASE
            WHEN
...
, days_weeks_info AS (
    SELECT
        ...
        , CASE
            WHEN
                trim(BOTH ' '::text FROM split_part(gs.stage, '-'::text, 2)) ~ '^\d+$'::text
                THEN nullif(trim(BOTH ' '::text FROM split_part(gs.stage, '-'::text, 2)), ''::text)::integer
            ELSE (gs.start_date_local::date - ssd.stage_start_date_local::date) / 7 + 1
