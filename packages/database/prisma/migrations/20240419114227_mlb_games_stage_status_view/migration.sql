CREATE OR REPLACE VIEW sport.mlb_games_stage_status_view AS WITH mlb_games_data AS (
        SELECT g.game_id,
            g.season_id,
            g.api_game_id,
            g.data,
        ...
        WHERE (
                g.api_source_id IN (
                    ...
                    WHERE lower(as2.api_source_name) ~~* '%baseball%'::text
                )
            )
    ),
    game_stages AS (
        SELECT mlb_games_data.data->>'id'::text AS api_game_id,
            (
                (
                    (mlb_games_data.data->>'date'::text)::timestamp WITH time zone
                ) AT TIME ZONE 'UTC'::text
            ) AS start_date,
            mlb_games_data.data->>'stage'::text AS stage
        FROM mlb_games_data
    ),
    stage_start_dates AS (
        SELECT mlb_games_data.data->>'stage'::text AS stage,
            (
                min(mlb_games_data.data->>'date'::text)::timestamp WITH time zone AT TIME ZONE 'UTC'::text
            ) AS stage_start_date
        FROM mlb_games_data
        GROUP BY (mlb_games_data.data->>'stage'::text)
    ),
    days_weeks_info AS (
