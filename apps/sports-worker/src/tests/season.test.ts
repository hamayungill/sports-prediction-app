...
import { seasonKafkaMessage } from '../utils/fixtures'
...
describe('Season', () => {
  ...
  it('should process the season message', async () => {
    ...
    expect(prismaClient.seasons.upsert).toHaveBeenCalledTimes(1)

    expect(logger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `${seasonKafkaMessage.sport_name}:Season:Upserted: ${JSON.stringify(seasonKafkaMessage.data)}`,
      ),
    )
  })

  it('should return an error while processing the season message', async () => {
    // @ts-expect-error We are mocking function so won't cause any issue
    prismaClient.seasons.upsert.mockRejectedValue(new Error('Error'))
    ...
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining(`${seasonKafkaMessage.sport_name}:Season:Error:`))
  })
})
