...
import { gameStatsKafkaMessage } from '../utils/fixtures'
...
describe('Games Stats', () => {
  ...
  it('should process the games stats message', async () => {
    // @ts-expect-error We are mocking function so won't cause any issue
    prismaClient.gamesStats.upsert.mockResolvedValue({})

    await startWorker()
    const kafkaMessage = {
      value: Buffer.from(JSON.stringify(gameStatsKafkaMessage)),
      key: null,
    ...
    expect(prismaClient.gamesStats.upsert).toHaveBeenCalledTimes(1)

    expect(logger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        `${gameStatsKafkaMessage.sport_name}:GameStats:Upserted: ${JSON.stringify(gameStatsKafkaMessage.data)}`,
      ),
    )
  })

  it('should return an error while processing the games stats message', async () => {
    // @ts-expect-error We are mocking function so won't cause any issue
    prismaClient.gamesStats.upsert.mockRejectedValue(new Error('Error'))
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining(`${gameStatsKafkaMessage.sport_name}:GameStats:Error:`),
    )
  })
})
