...
const imageName = 'mixpanel-worker'
const restart: boolean = checkDeploy.mixpanelWorker === 'true'
...
backendWorkerConfigMap.apply((v) => {
  const cm = k8s.core.v1.ConfigMap.get(
    'backend-workers-config-map-mixpanel-worker',
  ...
  createK8sDeployment({
    ...
    port: ports.mixpanelWorker,
  })
})

// Call the createService function
export const mixpanelWorker = createK8sService({
  name: imageName,
