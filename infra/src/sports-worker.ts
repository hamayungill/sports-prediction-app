...
const imageName = 'sports-worker'
const restart: boolean = checkDeploy.sportsWorker === 'true'
...
backendWorkerConfigMap.apply((v) => {
  const cm = k8s.core.v1.ConfigMap.get(
    'backend-workers-config-map-sports-worker',
  ...
  createK8sDeployment({
    ...
    port: ports.sportsWorker,
  })
})

// Create a Horizontal Pod Autoscaler for the deployment
export const hpa = new k8s.autoscaling.v1.HorizontalPodAutoscaler(
  `${imageName}-hpa`,
  {
    metadata: {
...
export const sportsWorker = createK8sService({
  name: imageName,
