...
import { k8sProvider, kafkaUrl, ports, backendWorkerNamepace, backendWorkerConfigMap, psql, opsgenieApiKey } from './utils/config'
import { checkDeploy } from './utils/const'

const imageName = 'alert-worker'
const restart: boolean = checkDeploy.alertWorker === 'true'
...
backendWorkerConfigMap.apply((v) => {
  const cm = k8s.core.v1.ConfigMap.get(
    'backend-workers-config-map-alert-worker',
  ...
  createK8sDeployment({
    ...
    env: [
      ...
      {
        name: 'OPSGENIE_DATA_API_KEY',
        value: opsgenieApiKey.data
      },
    ],
    port: ports.alertWorker,
  })
})

// Call the createService functions
export const alertWorker = createK8sService({
  name: imageName,
