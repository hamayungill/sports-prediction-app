...
const imageName = 'retry-worker'
const restart: boolean = checkDeploy.retryWorker === 'true'
...
backendWorkerConfigMap.apply((v) => {
  const cm = k8s.core.v1.ConfigMap.get(
    'backend-workers-config-map-retry-worker',
  ...
  createK8sDeployment({
    ...
    port: ports.retryWorker,
  })
})

// Call the createService function
export const retryWorker = createK8sService({
  name: imageName,
